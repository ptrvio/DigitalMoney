...
export const getUserActivities = (userId: string, token: string,limit?: number): Promise<Transaction[]> => {
  return fetch(
    myRequest(
...
export const getUserCards = (userId: string,token: string): Promise<Card[]> => {
  return fetch(myRequest(`${baseUrl}/users/${userId}/cards`, 'GET', token))
    ...
    .catch((err) => {
      console.log('ERROR AL OBTENER CARDS', err);
      //console.log(err);
      return rejectPromise(err);
    });
};
